...
const COL_CONFIG = {
  ...
  todo:        { label: "TODO",        color: "text-zinc-500",  border: "border-zinc-700",     dot: "bg-zinc-600" },
} as const;

function CardItem({ card, col }: { card: Card; col: keyof typeof COL_CONFIG }) {
  return (
    <div className={`p-2 bg-[#0a0a0b] border ${COL_CONFIG[col].border} rounded text-[11px] font-mono`}>
      <div className="text-zinc-300">{card.title}</div>
        ...
        <div className="mt-1.5 space-y-0.5">
          {card.subtasks.map((st) => (
            <div key={st.id} className="flex items-center gap-1.5 text-[10px]">
              <span className={`w-1 h-1 rounded-full ${st.status === "done" ? "bg-green-500" : st.status === "in_progress" ? "bg-amber-500" : st.status === "blocked" ? "bg-red-500" : "bg-zinc-600"}`} />
              <span className={st.status === "done" ? "text-zinc-600 line-through" : st.status === "blocked" ? "text-red-400/70" : "text-zinc-400"}>{st.title}</span>
            </div>
          ))}
        </div>
      ) : null}
...
export function KanbanBoard() {
  ...
  if (!entries.length) {
    return <div className="flex items-center justify-center h-full text-zinc-600 text-xs font-mono">No progress data</div>;
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 bg-[#0a0a0b] overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800 shrink-0">
        <div>
          <div className="text-[11px] font-mono text-amber-500">{p.phase}</div>
          <div className="text-[10px] text-zinc-500 mt-0.5">{p.current_task}</div>
        </div>
          ...
          return (
            <div key={col} className="flex-1 min-w-[200px] flex flex-col">
              <div className="flex items-center gap-1.5 px-1 pb-1.5 shrink-0">
                ...
                <span className="text-[10px] text-zinc-700 ml-auto">{cards.length}</span>
              </div>
